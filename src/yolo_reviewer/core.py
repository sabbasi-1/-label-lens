...
def replace_class_in_records(
    ...
    unsafe_records = [
        ...
        if any(issue.kind in unsafe_kinds for issue in record.issues)
    ]
    if unsafe_records:
        raise ValueError(
    ...
    operation_backup: Path | None = None
    if snapshots and backup_root is not None:
        operation_backup = (
            backup_root
            / ".label-lens-bulk-backups"
            / uuid.uuid4().hex
        )
    ...
    try:
        ...
        snapshot_by_id = {
            id(record): (boxes, issues)
            for record, boxes, issues, _original in snapshots
        }
        for record, _boxes, _issues, _original in snapshots:
            boxes, issues = snapshot_by_id[id(record)]
            record.boxes[:] = boxes
